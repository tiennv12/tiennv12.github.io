...
const [GET_LIST, CREATE, UPDATE, UPDATE_MANY, DELETE, DELETE_MANY, GET_MANY_REFERENCE, GET_ONE] = [
    'GET_LIST',
    'CREATE',
    'UPDATE',
    'UPDATE_MANY',
...
const dataProvider = (apiUrl, headers = () => {}, idParamApi = '_id', idParamAdmin = 'id', ftch = fetch) => {
    const getOptions = async (type, body) => {
        ...
        if (body) {
            options.body = JSON.stringify(_lodash.default.omit(body, [idParamApi, idParamAdmin]));
    ...
    const fn = async (type, resource, { pagination, sort, filter = {}, data, ids, id, target }) => {
        ...
        else result.data = Array.isArray(response) ? response.map(mapKeys) : mapKeys(response);
